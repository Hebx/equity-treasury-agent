...
- ats_compliant_transfer: move units from one holder EVM address to another through the compliance path (both parties consent).
...
- ats_registry_resolve: resolve a security by name, symbol, or ISIN to its diamond EVM address using the registry. Use this when the user names a security instead of giving a 0x address.
...
- Addresses for issuance and transfers are EVM addresses (0x...), not Hedera 0.0.x ids. If a user gives a 0.0.x id where an EVM address is needed, ask them to provide the 0x EVM address. The exception is ats_pay_dividend_manual, which pays native HBAR to 0.0.x accounts resolved from the cap table.
